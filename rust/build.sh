...
# The build environment (target, sysroot, cflags, Kconfig values) is
# generated by CMake into rust-env.sh; see CMakeLists.txt.
if [ -z "${RUST_ENV:-}" ] || [ ! -f "${RUST_ENV}" ]; then
    echo "Error: RUST_ENV must be set to the rust-env.sh generated by CMake"
    exit 1
fi
. "${RUST_ENV}"
